...
{
    public class MultiplayerMinifigInputManager : MonoBehaviour
    {
        MultiplayerMinifigController m_MinifigController;
        GameFlowManager m_GameFlowManager;

        void Awake()
        {
            m_MinifigController = GetComponent<MultiplayerMinifigController>();
        ...
        void OnGameOver(GameOverEvent evt)
        {
            ...
            if (evt.Win)
            {
                ...
                switch (randomCelebration)
                {
                    case 0:
                        {
                            m_MinifigController.PlaySpecialAnimation(MultiplayerMinifigController.SpecialAnimation.AirGuitar);
                            break;
                        }
                    case 1:
                        {
                            m_MinifigController.PlaySpecialAnimation(MultiplayerMinifigController.SpecialAnimation.Flexing);
                            break;
                        }
                    case 2:
                        {
                            m_MinifigController.PlaySpecialAnimation(MultiplayerMinifigController.SpecialAnimation.Dance);
                            break;
                        }
                }
            }
